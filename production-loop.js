...
function buildSymbolMap() {
    const map = {
        ...
        'Bullish': { 'BTCUSDC':'BTC_USDC','ETHUSDC':'ETH_USDC','SOLUSDC':'SOL_USDC' },
        // ─── Darkex (no SOL) ───
        'Darkex': { 'BTCUSDT':'BTC_USDT','ETHUSDT':'ETH_USDT' },
        // ─── Bitrue ───
...
const EXCHANGES = {
    ...
    // ═══════════════════════ Bullish (WS JSON-RPC, USDC pairs) ═══════════════════════
    // CRITICAL: OB and trades use SEPARATE WS endpoints
    // OB data is FLAT ARRAY: [price, qty, price, qty, ...]
    ...
    'Darkex': {
        ...
        onOpen: (ws) => {
            // SOL NOT LISTED on Darkex — only BTC and ETH
        ...
        parseMessage: (msg) => {
            try {
                const d = JSON.parse(msg);
                if (!d.channel || !d.tick) return [];
                const ch = d.channel;
                const m = ch.match(/market_(\w+?)_(depth|trade)/);
                if (!m) return [];
                const sym = m[1].toUpperCase();
                if (ch.includes('_depth_')) {
                    recordOrderbook('Darkex', sym, d.tick.buys || d.tick.bids, d.tick.asks);
                }
                if (ch.includes('_trade_') && d.tick.data) {
                    for (const t of d.tick.data) recordTrade('Darkex', sym, t.price, t.vol || t.amount, (t.side||'sell').toLowerCase());
                }
                return [];
            } catch(e) { return []; }
        }
    ...
    // ═══════════════════════ Bitrue (WS OB + REST trades, gzip) ═══════════════════════
    // WS has NO trade channels — all return {status:"error"}
    ...
    'Bitrue': {
        ...
        compression: 'gzip',
        handlePing: (parsed, ws) => {
            if (parsed.event === 'ping') { ws.send(JSON.stringify({ event: 'pong' })); return true; }
            return false;
        },
        onOpen: (ws) => {
        ...
        // REST polling for trades (Bitrue WS has NO trade channels)
        customPingSetup: (ws) => {
            const pollTrades = async () => {
                for (const sym of ['BTCUSDT','ETHUSDT','SOLUSDT']) {
                    try {
                        const resp = await httpsRequest(`https://openapi.bitrue.com/api/v1/trades?symbol=${sym}&limit=5`);
                        if (Array.isArray(resp)) {
                            for (const t of resp) recordTrade('Bitrue', sym, t.price, t.qty, t.isBuyerMaker ? 'sell' : 'buy');
                        }
                    } catch(e) {}
                }
            };
            pollTrades();
            return setInterval(pollTrades, 10000);
        },
        parseMessage: (msg) => {
            try {
                const d = JSON.parse(msg);
    ...
    'BloFin': {
        ...
        parseMessage: (msg) => {
            try {
                ...
                const sym = d.arg.instId;
                if (!sym) return [];
                if (d.arg.channel === 'trades' && d.data) {
                    for (const t of (Array.isArray(d.data) ? d.data : [d.data]))
    ...
    // ═══════════════════════ OrangeX (REST Deribit-style) ═══════════════════════

    'OrangeX': {
        tier: 3, type: 'rest',
        symbols: ['BTC-USDT','ETH-USDT','SOL-USDT'],
        endpoints: {
            orderbook: (sym) => `https://api.orangex.com/api/v1/public/get_order_book?instrument_name=${sym}-SPOT&depth=5`,
            trades: (sym) => `https://api.orangex.com/api/v1/public/get_last_trades_by_instrument?instrument_name=${sym}-SPOT&count=5`
        },
        parseOrderbook: (resp, sym) => {
            if (resp?.result && (resp.result.bids || resp.result.asks)) {
                recordOrderbook('OrangeX', sym, resp.result.bids, resp.result.asks);
            }
        },
        parseTrades: (resp, sym) => {
            if (resp?.result?.trades) {
                for (const t of resp.result.trades) recordTrade('OrangeX', sym, t.price, t.amount, t.direction);
            }
        }
    },

    // ═══════════════════════ Azbit (REST, data.azbit.com) ═══════════════════════

    'Azbit': {
        tier: 3, type: 'rest',
        symbols: ['BTC_USDT','ETH_USDT','SOL_USDT'],
        endpoints: {
            orderbook: (sym) => `https://data.azbit.com/api/orderbook?currencyPairCode=${sym}`,
            trades: (sym) => `https://data.azbit.com/api/deals?currencyPairCode=${sym}`
        },
        parseOrderbook: (resp, sym) => {
            if (Array.isArray(resp)) {
                const bids = resp.filter(d => d.isBid).slice(0,5).map(d => [d.price, d.amount]);
                const asks = resp.filter(d => !d.isBid).slice(0,5).map(d => [d.price, d.amount]);
                if (bids.length || asks.length) recordOrderbook('Azbit', sym, bids, asks);
            }
        },
        parseTrades: (resp, sym) => {
            if (Array.isArray(resp)) {
                for (const t of resp.slice(0,5)) recordTrade('Azbit', sym, t.price, t.volume, t.isBuy ? 'buy' : 'sell');
            }
        }
    },

    // ═══════════════════════ BVOX / BitVenus (REST Binance-style) ═══════════════════════

    'BVOX': {
        tier: 3, type: 'rest',
        symbols: ['BTCUSDT','ETHUSDT','SOLUSDT'],
        endpoints: {
            orderbook: (sym) => `https://api.bitvenus.me/openapi/quote/v1/depth?symbol=${sym}&limit=5`,
            trades: (sym) => `https://api.bitvenus.me/openapi/quote/v1/trades?symbol=${sym}&limit=5`
        },
        parseOrderbook: (resp, sym) => {
            if (resp?.bids && resp?.asks) recordOrderbook('BVOX', sym, resp.bids, resp.asks);
        },
        parseTrades: (resp, sym) => {
            if (Array.isArray(resp)) {
                for (const t of resp) recordTrade('BVOX', sym, t.price, t.qty, t.isBuyerMaker ? 'sell' : 'buy');
            }
        }
    },

    // ═══════════════════════ Trubit Pro (REST Binance-style) ═══════════════════════

    'Trubit Pro': {
        tier: 3, type: 'rest',
        symbols: ['BTCUSDT','ETHUSDT','SOLUSDT'],
        endpoints: {
            orderbook: (sym) => `https://api-spot.trubit.com/openapi/quote/v1/depth?symbol=${sym}&limit=5`,
            trades: (sym) => `https://api-spot.trubit.com/openapi/quote/v1/trades?symbol=${sym}&limit=5`
        },
        parseOrderbook: (resp, sym) => {
            if (resp?.bids && resp?.asks) recordOrderbook('Trubit Pro', sym, resp.bids, resp.asks);
        },
        parseTrades: (resp, sym) => {
            if (Array.isArray(resp)) {
                for (const t of resp) recordTrade('Trubit Pro', sym, t.price, t.qty, t.isBuyerMaker ? 'sell' : 'buy');
            }
        }
    },
    // ═══════════════════════════════════════════════════════════════════════════
...
async function testCCXTExchange(name) {
    ...
    let exchange;
    try {
        exchange = new ccxt.pro[exDef.ccxtId]({ newUpdates: true, enableRateLimit: true });
    } catch (e) {
...
async function testCCXTRestExchange(name) {
    ...
    let exchange;
    try {
        exchange = new ccxt[exDef.ccxtId]({ enableRateLimit: true });
    } catch (e) {
        console.log(`  [${ts()}] ❌ ${name} — CCXT init failed: ${e.message}`);
        stats[name].status = 'error';
        stats[name].errors.push(e.message);
